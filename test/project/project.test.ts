...
describe('Project API test', () => {
  describe('GET: All projects', () => {
    ...
    it('Should return list of projects', async () => {
      ...
      projectId = res.response.projects[1].id;
    });
  });
  ...
  describe('GET: A single project with project ID', () => {
    it('Should return 404 for org ID not found', async () => {
      try {
        const res = await Project.getSingleProject({ orgId: 'something-invalid', projectId });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
    ...
    it('Should return 400 for invalid project ID', async () => {
      try {
        const res = await Project.getSingleProject({ orgId: orgId1, projectId: 'test' });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
    ...
    it('Should return 401 for invalid auth token', async () => {
      try {
        const res = await Project.getSingleProject({ orgId: orgId1, projectId }, { apiToken: 'test' });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect401(errRes);
      }
  ...
  describe('DELETE: Delete project with the project ID', async () => {
    ...
    it('Should delete the project and return 200', async () => {
      ...
      const res = await Project.deleteAProject({orgId: orgId1, projectId: projectId});
  ...
  describe('GET: Get project deph graph by project ID', () => {
    it('Should return 404 for org ID not found', async () => {
      try {
        const res = await Project.getProjectDepGraph({ orgId: 'something-invalid', projectId });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect404(errRes);
      }
    ...
    it('Should return 404 for project ID not found', async () => {
      try {
        const res = await Project.getProjectDepGraph({ orgId: orgId1, projectId: 'test' });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect404(errRes);
      }
  ...
  describe('GET: List all ignores for a project', () => {
    it('Should return 404 for org ID not found', async () => {
      try {
        const res = await Project.listAllIgnores({ orgId: 'something-invalid', projectId });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect404(errRes);
      }
  ...
  describe("GET: Retrieve a ignore by it's issue id", () => {
    ...
    it('Should return 404 for org ID not found', async () => {
      try {
        const res = await Project.retrieveIgnore({ orgId: orgId1, projectId, issueId: 'some-kind-of-jiberish' });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect404(errRes);
      }
  ...
  */
  ...
  describe('GET: List Jira issues with project ID', () => {
    it('Should return 404 for org ID not found', async () => {
      try {
        const res = await Project.listAllJiraIssues({ orgId: 'something-invalid', projectId });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect404(errRes);
      }
    ...
    it('Should return 404 for project ID not found', async () => {
      try {
        const res = await Project.listAllJiraIssues({ orgId: orgId1, projectId: 'test' });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect404(errRes);
      }
  ...
  describe('GET: List project settings for a given project ID', () => {
    it('Should return 404 for org ID not found', async () => {
      try {
        const res = await Project.listProjectSettings({ orgId: 'test', projectId });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect404(errRes);
      }
    ...
    it('Should return 404 from project ID not found', async () => {
      try {
        const res = await Project.listProjectSettings({ orgId: orgId1, projectId: 'test' });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect404(errRes);
      }
  ...
  describe('DELETE: Project settings for given project ID', () => {
    it('Should return 404 for org ID not found', async () => {
      try {
        const res = await Project.deleteProjectSettings({ orgId: 'test', projectId });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect404(errRes);
      }
    ...
    it('Should return 404 for project ID not found', async () => {
      try {
        const res = await Project.deleteProjectSettings({ orgId: orgId1, projectId: 'test' });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect404(errRes);
      }
  ...
  describe('PUT: Move project on org to another', () => {
    it('Should throw an error if request body empty', async () => {
      try {
        const res = await Project.moveProject({ orgId: orgId1, projectId }, { requestBody: {} });
        utilFunctions.expectToNotExist(res);
      } catch (err) {
        expect(err).to.exist;
        expect(err).to.not.be.null;
    ...
    it('Should return 404 for source org ID not found', async () => {
      try {
        const res = await Project.moveProject(
          { orgId: 'something-invalid', projectId },
          { requestBody: { targetOrgId: orgId2 } },
        );
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect404(errRes);
    ...
    it('Should return 400 for target org ID not found', async () => {
      try {
        const res = await Project.moveProject({ orgId: orgId1, projectId }, { requestBody: { targetOrgId: 'test' } });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect400(errRes);
      }
  ...
  describe('POST: Add a tag to project', () => {
    ...
    it('Should return error for empty request body', async () => {
      try {
        const res = await Project.addATag({ orgId: orgId1, projectId }, { requestBody: {} });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
    ...
    it('Should return 404 for orgID not found', async () => {
      try {
        const res = await Project.addATag({ orgId: 'something-invalid', projectId }, { requestBody: reqBody });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect404(errRes);
      }
    ...
    it('Should return 404 for projectID not found', async () => {
      try {
        const res = await Project.addATag({ orgId: orgId1, projectId: 'test' }, { requestBody: reqBody });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
  ...
  describe('POST: Remove a trag from project', () => {
    ...
    it('Should return error for empty request body', async () => {
      try {
        const res = await Project.removeATag({ orgId: orgId1, projectId }, { requestBody: {} });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expectErr(errRes);
      }
    ...
    it('Should return 404 for orgID not found', async () => {
      try {
        const res = await Project.removeATag({ orgId: 'something-invalid', projectId }, { requestBody: reqBody });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect404(errRes);
      }
    ...
    it('Should return 404 for projectID not found', async () => {
      try {
        const res = await Project.removeATag({ orgId: orgId1, projectId: 'test' }, { requestBody: reqBody });
        utilFunctions.expectToNotExist(res);
      } catch (errRes) {
        utilFunctions.expect422(errRes);
      }
